...
description: Use when Agent Hub work requires the Human MCP server, especially Google Sheets or Google Docs read/write access. Load this before backlog-management when the live Agent Hub backlog must be inspected or changed.
---

# Skill: Human MCP Access

Use this skill to discover and call the repo-configured Human MCP server without guessing or substituting a local file.

## Canonical configuration

- MCP config: `.mcp.json`
- Server name: `human-mcp`
- Read the server URL from `.mcp.json`; do not duplicate or silently replace it elsewhere.
...
Use the Human MCP tools directly when they are exposed by the current client:
...
1. Call `sheets_read_rows` for the Agent Hub spreadsheet and `Backlog` tab.
2. Read the live header and map columns by name, never by assumed position.
3. Check existing IDs, titles, problems, dependencies, and implementation evidence before creating anything.
4. Deduplicate against existing rows. Do not invent a new ID until the live sheet has been checked.
5. Append or update only the agreed fields.
6. Re-read the sheet and verify the written row or cells before claiming success.
7. Follow `.skills/backlog-management/SKILL.md` for backlog content and governance rules.

## When MCP tools are not exposed directly

Do not conclude that Sheets access is unavailable until `.mcp.json` has been checked.

When the current environment exposes only local project command execution:

1. Read the Human MCP URL from `.mcp.json`.
...
- If Human MCP initialization, tool discovery, read, or write fails, stop and report the exact blocker.
